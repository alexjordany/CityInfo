...
public class CityInfoRepository : ICityInfoRepository
{
    ...
    public async Task<PointOfInterest?> GetPointOfInterestForCityAsync(int cityId, int pointOfInterestId) =>
        await _context.PointOfInterests.Where(p => p.CityId == cityId && p.Id == pointOfInterestId).FirstOrDefaultAsync();

    public async Task<IEnumerable<PointOfInterest>> GetPointsOfInterestsForCityAsync(int cityId) =>
        await _context.PointOfInterests.Where(p => p.CityId == cityId).ToListAsync();
